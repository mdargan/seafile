...
class SeafileClient(Project):
    ...
    def __init__(self):
        ...
        flags = {
            ...
            'CMAKE_PREFIX_PATH': conf[CONF_QT_ROOT],
...
class SeafileShellExt(Project):
    name = 'seafile-shell-ext-seafile'
    def __init__(self):
        Project.__init__(self)
        self.build_commands = [
            "bash extensions/build.sh",
...
def validate_args(usage, options):
    ...
    check_targz_src('seafile-shell-ext-seafile', seafile_shell_ext_version, srcdir)
...
def dependency_walk(applet):
    ...
    output = to_win_path(output)
    cmd = 'depends.exe -c -f:1 -oc:%s %s' % (output, to_win_path(applet))
...
def copy_shared_libs(exes):
    ...
    print('Copying shared libs')

    for lib in shared_libs:
        must_copy(lib, pack_bin_dir)

    print('Copying libssl')
    ...
    with open(file_path, 'r') as fp:
        for line in fp:
            if 'seafile-applet.exe' in line:
                ...
                new_line = re.sub('file_[\w]+', 'seafileapplet.exe', line)
...
def move_msi():
    ...
    print '---------------------------------------------'
    print 'The build is successfull. Output is:'
    print '>>\t%s' % dst_msi
    if not conf[CONF_ONLY_CHINESE]:
        print '>>\t%s' % dst_msi_en
        # print '>>\t%s' % dst_msi_de
